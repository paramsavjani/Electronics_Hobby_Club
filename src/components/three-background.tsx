...
type ThreeBackgroundProps = {
  ...
  droneEnabled?: boolean; // allow enabling/disabling drone group explicitly
};
...
export function ThreeBackground({
  ...
  speed = 0.06,
  colors,
  droneEnabled = true, // default drone visuals on (auto-downgrade for mobile/reduced motion below)
  ...
  React.useEffect(() => {
    ...
    // Respect reduced motion and detect environment for mobile/perf tuning
    const reduceMotion =
    ...
    // Clamp DPR harder on mobile / reduced motion to save battery
    renderer.setPixelRatio(
      Math.min(
        window.devicePixelRatio || 1,
        reduceMotion ? 1 : isMobile ? 1.5 : 2
      )
    );
    ...
    const c2 = colors?.[1]
      ...
      : c1.clone().offsetHSL(0.04, 0.1, 0.08); // slightly warmer/brighter
    const c3 = colors?.[2]
      ? new THREE.Color(colors[2])
      : c1.clone().offsetHSL(-0.06, -0.05, -0.06); // slightly cooler/darker
    ...
    const waves: Wave[] = [];
    {
      ...
      for (let i = 0; i < waveCount; i++) {
        ...
        const col =
          i % 2 === 0 ? colors?.[0] || "#22d3ee" : colors?.[1] || "#a3e635";
    ...
    // Throttle pointer events with rAF to reduce work on mobile
    let pmScheduled = false;
    let lastClientX = 0;
    let lastClientY = 0;
    const onPointerMoveScheduled = () => {
      pmScheduled = false;
    ...
    const chipCount = Math.max(
      8,
      Math.min(isMobile || reduceMotion ? 24 : 60, Math.floor(density / 120))
    );
    ...
    const enableDrone = droneEnabled && !reduceMotion;
    if (enableDrone) {
      const midZ = depth * 0.18;
      droneGroup.position.set(0, 0.05, midZ);
      world.add(droneGroup);

      const neonCyan = new THREE.Color(colors?.[0] || "#22d3ee");
      const neonOrange = new THREE.Color(colors?.[1] || "#ff7a1a");

      // helper to build a ring line
      const makeRing = (r: number, col: THREE.Color, alpha = 0.85) => {
        const segs = 64;
        ...
        const mat = new THREE.LineBasicMaterial({
          color: col,
          transparent: true,
          opacity: Math.min(1, opacity * alpha),
          blending: THREE.AdditiveBlending,
          depthWrite: false,
        });
      ...
      // body
      const body = makeRing(0.08, neonOrange, 0.9);
      droneGroup.add(body.ring);
      ...
      rotorOffsets.forEach((o) => {
        ...
        rg.position.set(o.x, o.y, 0);
        droneGroup.add(rg);

        const outer = makeRing(0.11, neonCyan, 0.85);
        const inner = makeRing(0.06, neonOrange, 0.85);
        rg.add(outer.ring);
        rg.add(inner.ring);
        rotorGroups.push({
          group: rg,
          outer: outer.ring,
          inner: inner.ring,
          geo: [outer.geo, inner.geo],
          mats: [outer.mat, inner.mat],
        });
      });

      // animate rotors via closure variables
      (droneGroup.userData as any).rotorGroups = rotorGroups;
      (droneGroup.userData as any).body = body;
      (droneGroup.userData as any).armMat = armMat;
    }

    // Animation loop
    const clock = new THREE.Clock();

    // Visibility-based start/stop to save battery on mobile
    const start = () => {
      ...
      const animate = () => {
        ...
        // Smooth parallax (slightly reduced on mobile)
        ...
        camera.position.x = parallaxX * (isMobile ? 5 : 8);
        camera.position.y = -parallaxY * (isMobile ? 4 : 6);
        camera.lookAt(0, 0, 0);

        // Subtle world rotation
        world.rotation.y = parallaxX * (isMobile ? 0.08 : 0.12);
        world.rotation.x = -parallaxY * (isMobile ? 0.05 : 0.08);
        ...
        for (let i = 0; i < groups.length; i++) {
          ...
          if (!reduceMotion) {
            const twinkle =
              0.9 + 0.1 * Math.sin(t * (0.8 + i * 0.3) + g.twinklePhase);
            g.mat.size = (i === 0 ? 2.2 : i === 1 ? 1.7 : 1.3) * twinkle;
            g.mat.opacity = Math.min(
              1,
              opacity *
                (i === 0 ? 0.85 : i === 1 ? 0.65 : 0.5) *
                (0.95 + 0.05 * Math.sin(t * 0.6 + i))
            );
          }
          ...
          for (let p = 0; p < count; p++) {
            ...
            if (!reduceMotion) {
              x +=
                Math.sin(t * (g.driftX + 0.05) + (z * 0.002 + (p % 7))) *
                0.0007;
              y +=
                Math.cos(t * (g.driftY + 0.04) + (z * 0.002 + (p % 11))) *
                0.0006;
            }
        ...
        if (!reduceMotion) {
          ...
          for (let i = 0; i < waves.length; i++) {
            ...
            for (let s = 0; s < count; s++) {
              ...
              w.pos.array[idx + 1] =
                w.baseY + w.amp * Math.sin(w.freq * (x + w.phase));
              // z remains constant
            }
            ...
            lineMat.opacity = Math.min(
              1,
              opacity * (0.35 - i * 0.05) * (0.9 + 0.1 * Math.sin(t * 1.3 + i))
            );
          }
          ...
          if (nodeMat && lineMat) {
            const pulse =
              0.92 + 0.08 * Math.sin(t * 1.1 + circuit.twinklePhase);
            nodeMat.size = 2.2 * pulse;
            nodeMat.opacity = Math.min(
              1,
              opacity * 0.9 * (0.9 + 0.1 * Math.sin(t * 0.9))
            );
            lineMat.opacity = Math.min(
              1,
              opacity * 0.45 * (0.9 + 0.1 * Math.cos(t * 1.0))
            );
          }
        }
        ...
        if (enableDrone) {
          const rotorGroups = (droneGroup.userData as any).rotorGroups as {
            group: THREE.Group;
            outer: THREE.LineLoop;
            inner: THREE.LineLoop;
            geo: THREE.BufferGeometry[];
          ...
          rotorGroups.forEach((rg, i) => {
            rg.group.rotation.z += dt * 0.5 * (i % 2 === 0 ? 1 : -1);
          });
        }
    ...
    // Hook up observers for visibility
    ...
    return () => {
      ...
      if (enableDrone) {
        ...
        rotorGroups?.forEach((rg) => {
          rg.geo.forEach((g) => g.dispose());
          rg.mats.forEach((m) => m.dispose());
          droneGroup.remove(rg.group);
        });
        const body = (droneGroup.userData as any).body as {
          ring: THREE.LineLoop;
          geo: THREE.BufferGeometry;
        ...
        if (body) {
          body.geo.dispose();
          body.mat.dispose();
          droneGroup.remove(body.ring);
        }
        const armMat = (droneGroup.userData as any)
          .armMat as THREE.LineBasicMaterial;
        armMat?.dispose();
        world.remove(droneGroup);
      }
  ...
  }, [colors, density, depth, opacity, speed, droneEnabled]);
